...
cat >/etc/nginx/conf.d/xray.conf <<EOF
    server {
             ...
             listen [::]:2096 http2;
             listen 8433 ssl http2 reuseport;
             listen [::]:8433 http2 reuseport;
             server_name $domain;
...
EOF
sed -i '$ ilocation = /zhee-vless' /etc/nginx/conf.d/xray.conf
sed -i '$ i{' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_redirect off;' /etc/nginx/conf.d/xray.conf
sed -i '$ iproxy_pass http://127.0.0.1:10000;' /etc/nginx/conf.d/xray.conf
